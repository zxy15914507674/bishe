...
public class ClientWeightObjectMethod : NetworkBehaviour {
    ...
    void Update()
    {
        if (isClient)
        {
            ...
            if (objHandControl.hasCatched != null && objHandControl.hasCatched.name == "tweezer")
            {
                //通过本地游戏物体发送当前游戏物体同步位置和旋转信息到服务端执行，进入同步到各个客户端
                SendToServerByPlayer objSend = localPlayer.GetComponent<SendToServerByPlayer>();
                v3_Pos = transform.position;
                qua_Rotate = transform.eulerAngles;
                objSend.CmdWeightMove(gameObject.name, v3_Pos, qua_Rotate);
