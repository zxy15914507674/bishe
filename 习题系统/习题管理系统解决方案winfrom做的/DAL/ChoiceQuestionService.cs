...
namespace DAL
{
    ...
    public class ChoiceQuestionService
    {
        ...
        {
            string sql = @"insert into ChoiceQuestion(expName,sceneName,questionTypeNumber,content,optionA,optionB,optionC,optionD,picture,answer,score,teacherName,tipMessage,thinkTime)
            ...
            sql = string.Format(sql, choiceQuestion.expName,choiceQuestion.sceneName, choiceQuestion.questionTypeNumber, choiceQuestion.content,
                                  ...
                                  choiceQuestion.tipMessage,choiceQuestion.thinkTime
        ...
        {
            string sql = @"select expName,sceneName,questionTypeNumber,content,optionA,optionB,optionC,optionD,picture,answer,score,teacherName,
                           tipMessage,thinkTime
                           from ChoiceQuestion where id="+id;
        ...
        public int ModifyChoiceQueston(ChoiceQuestion choiceQuestion)
        {
            ...
            sql += @"optionB='{4}',optionC='{5}',optionD='{6}',picture='{7}',answer='{8}',score='{9}',teacherName='{10}',
                     tipMessage='{11}',thinkTime={12}
                    where id={13}";

            sql = string.Format(sql,choiceQuestion.expName,choiceQuestion.sceneName,choiceQuestion.content,choiceQuestion.optionA,
                choiceQuestion.optionB,choiceQuestion.optionC,choiceQuestion.optionD,choiceQuestion.picture,choiceQuestion.answer,choiceQuestion.score,choiceQuestion.teacherName,
                choiceQuestion.tipMessage,choiceQuestion.thinkTime,
                choiceQuestion.id
                );
